...
export default function EditEventModal({ event, onClose, onSuccess }: EditEventModalProps) {
  const [form, setForm] = useState<CreateEventPayload>(eventToPayload(event));
  ...
  function handleChange(
    ...
    setForm((prev) => {
      ...
      if (name === 'all_day' && checked) updated.date_end = '';
      return updated;
    });
  }
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input type="text" name="theme" value={form.theme} onChange={handleChange} placeholder="Ex: Tecnologia, Negócios..." className={inputClass} />
          </div>
          ...
          <div className="border border-gray-200 rounded-lg p-3 space-y-3 bg-gray-50">
            <div className="flex items-center gap-2">
              <input
                type="checkbox" id="edit_all_day" name="all_day"
                checked={form.all_day} onChange={handleChange}
                className="w-4 h-4 text-blue-600 rounded border-gray-300"
              />
              <label htmlFor="edit_all_day" className="text-sm font-medium text-gray-700 cursor-pointer">
            ...
            <div className={`grid gap-3 ${form.all_day ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2'}`}>
              <div>
                ...
                <input
                  ...
                  name="date_start" value={form.date_start} onChange={handleChange} required className={inputClass}
                />
              </div>
              {!form.all_day && (
                <div>
                  ...
                  <input
                    type="datetime-local" name="date_end"
                    value={form.date_end} onChange={handleChange}
                    min={form.date_start || undefined} className={inputClass}
                  />
                </div>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <input type="text" name="location" value={form.location} onChange={handleChange} placeholder="Endereço ou link online" className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>Organizador</label>
              <input type="text" name="organizer" value={form.organizer} onChange={handleChange} placeholder="Nome do organizador" className={inputClass} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className={labelClass}>Decisão <span className="text-red-500">*</span></label>
          ...
          <div>
            ...
            <input
              type="text" name="attendees" value={form.attendees} onChange={handleChange}
              placeholder="email1@ex.com, email2@ex.com" className={inputClass}
            />
          ...
          <div>
            ...
            <textarea name="description" value={form.description} onChange={handleChange} rows={2} placeholder="Detalhes do evento..." className={`${inputClass} resize-none`} />
          </div>

          {/* Observações */}
          <div>
            <label className={labelClass}>Observações</label>
            <textarea name="notes" value={form.notes} onChange={handleChange} rows={2} placeholder="Notas pessoais..." className={`${inputClass} resize-none`} />
          </div>
          ...
          <div className="flex gap-3 pt-2">
            <button
              ...
              className="flex-1 border border-gray-300 text-gray-700 hover:bg-gray-50 font-medium py-2 px-4 rounded-md text-sm transition-colors"
            >
              Cancelar
            </button>
            <button
              type="submit" disabled={loading}
              className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-medium py-2 px-4 rounded-md text-sm transition-colors"
            >
